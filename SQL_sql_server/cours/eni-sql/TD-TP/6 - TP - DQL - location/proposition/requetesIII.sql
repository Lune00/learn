...
WHERE cpo IN (44000, 44100, 44200, 44300);
...
SELECT	l.noFic [numéro de fiche],
		...
		montant=(DATEDIFF(DAY, depart, ISNULL(retour, GETDATE()))+1)*prixJour
FROM fiches f
...
SELECT	l.noFic [numéro de fiche],
		...
		ISNULL(CONVERT(VARCHAR, retour, 103), '-') retour,
		prixJour AS [prix par jour],
		(DATEDIFF(DAY, depart, ISNULL(retour, GETDATE()))+1)*prixJour montant
FROM fiches f
	INNER JOIN clients c ON f.noCli = c.noCli
	INNER JOIN lignesFic l ON f.noFic = l.noFic
	INNER JOIN articles a ON l.refart = a.refart
	...
	INNER JOIN tarifs t	ON g.codeTarif = t.codeTarif
WHERE l.noFic = 1002
COMPUTE SUM((DATEDIFF(DAY, depart, ISNULL(retour, GETDATE()))+1)*prixJour);
